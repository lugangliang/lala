			...
			inputRules = append(inputRules,
				Rule{
					Match: Match().ProtocolNum(ProtoUDP).
						...
						DestAddrType(AddrTypeLocal),
					Action:  r.filterAllowAction,
					Comment: []string{"Allow VXLAN IPv4 packets from whitelisted hosts"},
				},)
		}
			...
			inputRules = append(inputRules,
				...
				Rule{
					Match: Match().ProtocolNum(ProtoUDP).
						...
						DestAddrType(AddrTypeLocal),
					Action:  r.filterAllowAction,
					Comment: []string{"Allow VXLAN IPv6 packets from whitelisted hosts"},
				},)
		}
...
func (r *DefaultRuleRenderer) StaticNATPostroutingChains(ipVersion uint8) []*Chain {
	...
	if r.VXLANEnabled {
		if len(r.VXLANTunnelIPv4Address) > 0 {
			tunnelIfaces = append(tunnelIfaces, "vxlan.calicoV4")
		}
		if len(r.VXLANTunnelIPv6Address) > 0 {
			tunnelIfaces = append(tunnelIfaces, "vxlan.calicoV6")
		}
